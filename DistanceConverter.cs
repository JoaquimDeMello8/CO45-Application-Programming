...
namespace DistanceConverter
{
    class Program
    {
        ...
        {   //Title 
            ...
            int unit = 0;
            //If user does not pick between 1 and 6 then error message will be displayed
            if (!int.TryParse(unitString, out unit) || (unit < 1 || unit > 6))
            {
                Console.WriteLine("Error: Invalid unit choice entered.");
                return;
            }
            //Asks the user to choose the To unit
            ...
            //All combinations that the user could pick and displays the corresponding one
